...
from pkm_bridge.database import close_db, get_db, init_db
...
def test_database():
    ...
    try:
        ...
        session = SessionRepository.create_session(
            db=db, session_id="test-session-123", system_prompt="You are a helpful assistant."
        )
        print(f"   ✓ Created session: {session}")

        # Append messages
        session = SessionRepository.append_message(
            db=db, session_id="test-session-123", role="user", content="Hello!"
        )
        ...
        assert SessionRepository.get_session(db, "test-session-123") is not None
